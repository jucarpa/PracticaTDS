...
public class PanelLogin extends JPanel{
	...
	private void crearPantalla() {
		...
		lalerta2.setForeground(Color.RED); fixedSize(lalerta2,Constantes.x_size,30);
		lalerta2.setVisible(false);
		
		lalerta3=new JLabel("Usuario no existe. ",JLabel.CENTER);
		lalerta3.setForeground(Color.RED); fixedSize(lalerta3,Constantes.x_size,30);
		...
		btnLogin.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if (auxUsuario.isEmpty()||auxContrasena.isEmpty()) lalerta.setVisible(true);
				else { lalerta.setVisible(false);
						if(!ControladorAppChat.getUnicaInstancia().existeUsuario(auxUsuario)) lalerta3.setVisible(true);
						else {
							if(!ControladorAppChat.getUnicaInstancia().loginUsuario(auxUsuario, auxContrasena)) lalerta2.setVisible(true);
							else {
